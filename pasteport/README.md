...
Na colagem, o PastePort lê apenas `event.clipboardData.items` após a ação explícita do usuário. Itens de imagem são transformados em novos objetos `File` com nomes como:
...
- O conteúdo do clipboard não é armazenado.
- O clipboard não é monitorado continuamente.
- A extensão acessa uma imagem somente por meio do evento `paste` iniciado pelo usuário.
- Não há telemetria ou coleta de dados.
- Não há scripts, estilos, fontes ou bibliotecas remotas.
- Não há `eval`.
- As imagens deixam de ser referenciadas pela extensão após a entrega ao input e o fechamento do modal.
